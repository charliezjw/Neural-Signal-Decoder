...
conv_total = tf.concat([conv_1, conv_2, conv_3], axis=3)
pool1 = tf.layers.average_pooling2d(inputs=conv_total, pool_size=[1, 10], strides=[1, 10])
...
lstm_cell = rnn.BasicLSTMCell(1024)
outputs, states = rnn.static_rnn(lstm_cell, x, dtype=tf.float32)

fc_5 = tf.layers.dense(inputs=outputs[-1], units=1024, activation=tf.nn.relu, kernel_regularizer=regularizer)
